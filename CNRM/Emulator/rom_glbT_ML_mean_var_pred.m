...
function data_pred = ...
    rom_glbT_ML_mean_var_pred(data_est,t_est,t_pred,mu_pred,var_pred,mu_valid,var_valid)
...
% 
%         Xtrain = T_glb_est_season{k}(1:skip(k):end);
%         Ytrain = mu_est_season{k}(1:skip(k):end,j);
%         error_train = zeros(10,1);
%         for l=1:20
%             Mdl{l} = fitrnet(Xtrain,Ytrain,'LayerSizes',[3,3],...
%                 'Standardize',true,'Activations','relu');
%             error_train(k) = loss(Mdl{k},Xtrain,Ytrain);
%         end
%         [~,ind] = min(error_train);
%         trainPredictions = predict(Mdl{ind},T_glb_est_season{k}(:));
%         mu_pred_season{k}(:,j) = trainPredictions;
% 
%         Xtrain = T_glb_est_season{k}(1:skip(k):end);
%         Ytrain = var_est_season{k}(1:skip(k):end,j);
%         error_train = zeros(10,1);
%         for l=1:20
%             Mdl{k} = fitrnet(Xtrain,Ytrain,'LayerSizes',[3,3],...
%                 'Standardize',true,'Activations','relu');
%             error_train(k) = loss(Mdl{k},Xtrain,Ytrain);
%         end
%         [~,ind] = min(error_train);
%         trainPredictions = predict(Mdl{ind},T_glb_est_season{k}(:));
%         var_pred_season{k}(:,j) = trainPredictions;
%     end
% end
% 
...
% if(~isempty(idx))
%     fprintf('negative variance found at %d, %d',idx_1,idx_2);
%     return;
% end
