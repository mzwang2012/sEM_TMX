...
function data_pred = rom_LE_glbT_lin_mean_var_pred(data_est,T_glb,T_glb_valid,t_est,t_pred,n_rlz)
...
disp('Compute covariance and Cholesky decomposition');
...
for i=1:4
    ...
    for l=1:n_ens
        corr_season(:,:,i) = corr_season(:,:,i) + ...
            data_est_season{i}(:,:,l)' * data_est_season{i}(:,:,l) / l1;
    end
    corr_season(:,:,i) = corr_season(:,:,i) / n_ens;
...
for i=1:4
    for l=1:n_ens
        tmp = L_inv_season(:,:,i) * data_est_season{i}(:,:,l)';
        %data_est_noise{i}(:,:,l) = tmp';  % w_s(t) in Eq. (10)
        data_est_season{i}(:,:,l) = tmp';  % w_s(t) in Eq. (10)
    end
end
...
for k=1:4   
    ...
    for l=1:n_rlz
        tmp = L_season(:,:,k) * data_pred_season{k}(:,:,l)';
        data_pred_season{k}(:,:,l) = tmp';
    end
end
